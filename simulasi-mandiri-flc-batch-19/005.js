...
function simpleCardsFusion (str) {
  ...
  var arrKartu = str.split('+');
  for (var i = 0; i< fusionCards.length; i++){
    // console.log(fusionCards[i].name,i )
    if (fusionCards[i].card1 === arrKartu[0] && fusionCards[i].card2 === arrKartu[1]){
      return fusionCards[i].name
    }

    if (fusionCards[i].card1 === arrKartu[1] && fusionCards[i].card2 === arrKartu[0]){
      return fusionCards[i].name;
    }
    // if ((fusionCards[i].card1 === arrKartu[0] || fusionCards[i].card1 === arrKartu[1]) && (fusionCards[i].card2 === arrKartu[0] || fusionCards[i].card2 === arrKartu[1])){
    //     return fusionCards[i].name;
    // }
  }
  // for (var key in fusionCards){
